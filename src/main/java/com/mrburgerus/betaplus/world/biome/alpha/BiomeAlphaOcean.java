...
public class BiomeAlphaOcean extends AbstractAlphaBiome
{
	...
	{
		super((new BiomeBuilder()).surfaceBuilder(new CompositeSurfaceBuilder<>(DEFAULT_SURFACE_BUILDER, GRASS_DIRT_GRAVEL_SURFACE)).precipitation(RainType.RAIN).category(Category.OCEAN).depth(0.0F).scale(0.0F).temperature(0.5F).downfall(0.5F).waterColor(WATER_COLOR).waterFogColor(329011).parent((String)null));
