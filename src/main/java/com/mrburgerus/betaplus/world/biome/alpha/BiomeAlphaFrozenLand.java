...
public class BiomeAlphaFrozenLand extends AbstractAlphaBiome
{
	...
	public BiomeAlphaFrozenLand()
	{
		...
		super((new Biome.BiomeBuilder()).surfaceBuilder(new CompositeSurfaceBuilder<>(DEFAULT_SURFACE_BUILDER, GRASS_DIRT_SAND_SURFACE)).precipitation(RainType.SNOW).category(Category.PLAINS).depth(0.0F).scale(0.1F).temperature(0.0F).downfall(100.0F).waterColor(WATER_COLOR).waterFogColor(329011).parent((String)null));
