...
import com.mrburgerus.betaplus.world.beta_plus.util.AbstractWorldSimulator;
...
public class BetaPlusSimulator extends AbstractWorldSimulator
{
	...
	protected Pair<int[][], Boolean> simulateChunkYFastOrig(Chunk pos)
	{
		int[][] output = new int[16][16];
		heightNoise = generateOctaves(heightNoise, pos.getPos().x * 4, 0,pos.getPos().z * 4, 5, 17, 5);
		for (int cX = 0; cX < 4; ++cX)
		{
			for (int cZ = 0; cZ < 4; ++cZ)
			{
